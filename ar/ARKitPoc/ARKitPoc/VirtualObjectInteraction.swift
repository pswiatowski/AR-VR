...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    func didLongPress(_ gesture: UILongPressGestureRecognizer) {
        ...
        trackedObject?.geometry?.materials.forEach({ $0.multiply.contents = UIColor.red })
        SCNTransaction.commit()
        //trackedObject?.geometry?.materials.forEach({ $0.diffuse.contents = UIColor.red })
        
        /*let touchLocation = gesture.location(in: sceneView)
        if let tappedObject = sceneView.virtualObject(at: touchLocation) {
            tappedObject.geometry?.materials.forEach({ $0.diffuse.contents = UIColor.red })
        } else if let object = selectedObject {
